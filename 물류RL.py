...
epoch = 20  # 에포크 수: PPO 학습 시 각 업데이트마다 20번의 에포크를 수행(에포크는 경험상 크게 건들지 않아도 됨. 에포크를 조정하면 후술할 맥스스텝과 total_timesteps을 조정해야해서 번거로움)
...
total_timesteps = 200000  # 총 학습 스텝 수: PPO 학습 시 전체 학습 스텝 수
...
class VRPEnv(gym.Env):
    ...
    def step(self, action):
        ...
        delivery = 0  # 배송량 초기화
        max_dist = self.dist_matrix.max().item() # 최대 거리: 현재 거리 행렬에서 최대값을 가져옴
        reward = -(dist / max_dist) * 10.0 # 이동 거리로 인한 보상 계산 (거리가 멀수록 보상이 감소) 정규화거리에 특정 보상계수를 곱하여 보상으로 사용
        ...
        if self.initial_unbalance > 0: # 초기 불균형이 0보다 큰 경우
            reward += (delta / self.initial_unbalance) * 50.0 # 초기 불균형 대비 변화량에 따라 보상을 조정
        reward -= 5 # 매 스텝마다 일정한 감소 보상 적용(스텝을 최대한 간소화 하기 위해)
