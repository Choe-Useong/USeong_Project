...
class InputPage(QWidget):
    ...
    def run(self):
        ...
        config = {
            "API_KEY": API_KEY,                                # 상단에 이미 있음
            "REGION_LIST": self.region_list,          # MainWindow에서 받은 지역
            "START_ADDRESS": df_input.iloc[0]["address"],      # 예시: 첫 주소를 출발지로 설정
            "ITEM_COLS": ITEM_COLS,
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.region_list = []  # ← 저장소 초기화
        self.region_page = RegionPage(self.on_region_confirm)
        self.input_page = None  # ← 초기에는 None
        self.stack.addWidget(self.region_page)
        self.setCentralWidget(self.stack)

    def on_region_confirm(self, region_list):
        self.region_list = region_list  # ← 지역 리스트 저장
        self.input_page = InputPage(region_list)  # ← 전달
